  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-900">产品套餐</h1>
          <p className="text-sm text-slate-500 mt-1">将多个产品组合为套餐，设置组合折扣</p></div>
        <button onClick={() => { setFormError(""); setShowForm(true); }} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700"><Plus className="w-5 h-5" /> 新增套餐</button>
      </div>
          ...
          : data.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            : <table className="w-full"><thead><tr className="bg-slate-50 text-left">
              ...
              {data.map((p) => <tr key={p.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-900">{p.parentProduct.name}</td>
                <td className="px-4 py-3 text-slate-600">{p.childProduct?.name || "-"}</td>
                <td className="px-4 py-3 text-slate-700">¥{p.childProduct?.price || 0}</td>
                <td className="px-4 py-3 text-slate-600">{p.discount != null ? p.discount : "-"}</td>
                <td className="px-4 py-3"><button onClick={() => setDeleteConfirm(p)} className="p-1.5 text-gray-400 hover:text-red-600 rounded"><Trash2 className="w-5 h-5" /></button></td>
              </tr>)}
            </tbody></table>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div><label className="block text-sm font-medium text-slate-700 mb-1">父产品（套餐）</label>
                <select value={form.parentProductId} onChange={(e) => setForm({ ...form, parentProductId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                  <option value="">请选择</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">子产品（捆绑项）</label>
                <select value={form.childProductId} onChange={(e) => setForm({ ...form, childProductId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                  <option value="">请选择</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">折扣(%)</label>
                <input value={form.discount} onChange={(e) => setForm({ ...form, discount: e.target.value })} type="number" step="0.01" placeholder="如 10 表示九折" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" /></div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6"><p className="text-sm text-slate-700">确定删除套餐 <strong>{deleteConfirm.parentProduct.name} + {deleteConfirm.childProduct?.name}</strong>？</p>
