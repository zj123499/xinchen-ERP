  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-900">院校管理</h1>
          <p className="text-sm text-slate-500 mt-1">维护院校库，支撑选校与报价</p></div>
        <button onClick={openNew} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700"><Plus className="w-5 h-5" /> 新增院校</button>
      </div>
        ...
        {loading ? <div className="flex items-center justify-center py-20 text-gray-400"><RefreshCw className="w-5 h-5 animate-spin mr-2" />加载中...</div>
          : data.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-gray-400"><p className="text-sm">暂无院校数据</p>
            ...
            : <table className="w-full"><thead><tr className="bg-slate-50 text-left">
              ...
              {data.map((i) => <tr key={i.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-900">{i.name}</td>
                <td className="px-4 py-3 text-slate-600">{i.country?.name}</td>
                ...
                <td className="px-4 py-3 text-slate-600">{i.ranking ?? "-"}</td>
                <td className="px-4 py-3 text-slate-600">{i.tuitionRange || "-"}</td>
                <td className="px-4 py-3 text-slate-600">{i._count.majors}</td>
                <td className="px-4 py-3"><div className="flex items-center gap-1">
                  <button onClick={() => openEdit(i)} className="p-1.5 text-gray-400 hover:text-blue-600 rounded"><Edit2 className="w-5 h-5" /></button>
                  <button onClick={() => setDeleteConfirm(i)} className="p-1.5 text-gray-400 hover:text-red-600 rounded"><Trash2 className="w-5 h-5" /></button>
                </div></td>
              </tr>)}
            </tbody></table>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div><label className="block text-sm font-medium text-slate-700 mb-1">院校名称 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700 mb-1">国家 <span className="text-red-500">*</span></label>
                  <select required value={form.countryId} onChange={(e) => setForm({ ...form, countryId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                ...
                <div><label className="block text-sm font-medium text-slate-700 mb-1">类型</label>
                  <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                    {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700 mb-1">排名</label>
                  <input value={form.ranking} onChange={(e) => setForm({ ...form, ranking: e.target.value })} type="number" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" /></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-1">学费区间</label>
                  <input value={form.tuitionRange} onChange={(e) => setForm({ ...form, tuitionRange: e.target.value })} placeholder="如 3-5万/年" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" /></div>
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6"><p className="text-sm text-slate-700">确定删除院校 <strong>{deleteConfirm.name}</strong>？相关专也将一并删除。</p>
