  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">收款管理</h1>
          <p className="text-sm text-slate-500 mt-1">管理所有收款记录</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <select value={paymentTypeFilter} onChange={(e) => { setPaymentTypeFilter(e.target.value); setPage(1); }}
            className="px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none">
          ...
          <button onClick={() => { setKeyword(""); setSearchKeyword(""); setPaymentTypeFilter(""); setPage(1); }}
            className="p-2 text-gray-400 hover:text-slate-600 hover:bg-gray-100 rounded-lg transition" title="刷新">
                  ...
                  <tr key={pmt.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-sm font-medium text-slate-900">{pmt.student.name}</div>
                          <div className="text-xs text-slate-400">{pmt.student.phone}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-700">{PAYMENT_TYPE_MAP[pmt.paymentType] || pmt.paymentType}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="text-sm font-semibold text-slate-900">{formatAmount(pmt.amount, pmt.currency)}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-600">{PAYMENT_METHOD_MAP[pmt.method] || pmt.method}</span>
                    </td>
                    <td className="px-6 py-4">
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t border-slate-200 bg-slate-50">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
                  className="p-1.5 text-gray-500 hover:bg-gray-200 rounded disabled:opacity-30 disabled:cursor-not-allowed transition"><ChevronLeft className="w-5 h-5" /></button>
                {Array.from({ length: Math.min(data.totalPages, 5) }, (_, i) => {
                  ...
                  return <button key={pn} onClick={() => setPage(pn)} className={`w-8 h-8 text-sm rounded transition ${pn === page ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-gray-200"}`}>{pn}</button>;
                })}
                <button onClick={() => setPage((p) => Math.min(data.totalPages, p + 1))} disabled={page >= data.totalPages}
                  className="p-1.5 text-gray-500 hover:bg-gray-200 rounded disabled:opacity-30 disabled:cursor-not-allowed transition"><ChevronRight className="w-5 h-5" /></button>
              </div>
            </div>
          </>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                {selectedStudent ? (
                  ...
                  <div className="relative">
                    <input type="text" value={studentKeyword} onChange={(e) => searchStudents(e.target.value)}
                      placeholder="搜索学生姓名或手机号..." className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">收款类型</label>
                ...
                <div>
                  ...
                  <input type="number" step="0.01" value={form.amount} onChange={(e) => setForm((f) => ({ ...f, amount: e.target.value }))}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">付款人</label>
                <input type="text" value={form.payerName} onChange={(e) => setForm((f) => ({ ...f, payerName: e.target.value }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="付款人姓名" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">关联订单</label>
                <input type="text" value={form.orderId} onChange={(e) => setForm((f) => ({ ...f, orderId: e.target.value }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="输入订单ID（可选）" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">备注</label>
                <textarea value={form.remark} onChange={(e) => setForm((f) => ({ ...f, remark: e.target.value }))} rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" placeholder="备注信息" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              ...
              <button onClick={() => setShowDetail(false)} className="p-1 text-gray-400 hover:text-slate-600">✕</button>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                <div className="text-xs text-slate-400">收款编号</div>
                <div className="text-sm font-mono font-medium text-slate-900">{detailPayment.paymentNo}</div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><div className="text-xs text-slate-400 mb-1">学生</div><div className="text-sm font-medium text-slate-900">{detailPayment.student.name}</div></div>
                <div><div className="text-xs text-slate-400 mb-1">电话</div><div className="text-sm text-slate-700">{detailPayment.student.phone}</div></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><div className="text-xs text-slate-400 mb-1">收款类型</div><div className="text-sm text-slate-700">{PAYMENT_TYPE_MAP[detailPayment.paymentType] || detailPayment.paymentType}</div></div>
                <div><div className="text-xs text-slate-400 mb-1">金额</div><div className="text-sm font-semibold text-slate-900">{formatAmount(detailPayment.amount, detailPayment.currency)}</div></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><div className="text-xs text-slate-400 mb-1">收款方式</div><div className="text-sm text-slate-700">{PAYMENT_METHOD_MAP[detailPayment.method] || detailPayment.method}</div></div>
                <div><div className="text-xs text-slate-400 mb-1">收款日期</div><div className="text-sm text-slate-700">{new Date(detailPayment.paidAt).toLocaleDateString("zh-CN")}</div></div>
              </div>
              {detailPayment.exchangeRate && Number(detailPayment.exchangeRate) !== 1 && (
                <div className="grid grid-cols-2 gap-4">
                  <div><div className="text-xs text-slate-400 mb-1">汇率</div><div className="text-sm text-slate-700">{String(detailPayment.exchangeRate)}</div></div>
                  <div><div className="text-xs text-slate-400 mb-1">本位币金额</div><div className="text-sm font-medium text-slate-900">{formatAmount(String(detailPayment.baseAmount || detailPayment.amount), "CNY")}</div></div>
                </div>
              )}
              {detailPayment.payerName && (
                <div><div className="text-xs text-slate-400 mb-1">付款人</div><div className="text-sm text-slate-700">{detailPayment.payerName}</div></div>
              )}
              {detailPayment.order && (
                <div><div className="text-xs text-slate-400 mb-1">关联订单</div>
              ...
              {detailPayment.remark && (
                <div><div className="text-xs text-slate-400 mb-1">备注</div><div className="text-sm text-slate-700 whitespace-pre-wrap">{detailPayment.remark}</div></div>
              )}
              <div className="text-xs text-slate-400 pt-2 border-t border-gray-100">
