  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">记录用户在系统中的关键操作（增删改、导出、导入等），便于行为追溯</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <input value={keyword} onChange={(e) => { setKeyword(e.target.value); setPage(1); }}
            placeholder="搜索操作对象"
            className="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none w-52" />
          <button onClick={() => { setModuleFilter(""); setActionFilter(""); setKeyword(""); setPage(1); }}
            className="p-2 text-gray-400 hover:text-slate-600 hover:bg-gray-100 rounded-lg transition" title="重置"><RefreshCw className="w-5 h-5" /></button>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.list.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-50 transition">
                    <td className="px-4 py-3 text-sm font-medium text-slate-900">{l.operatorName}</td>
                    <td className="px-4 py-3 text-sm text-slate-600">{l.module}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${ACTION_COLOR[l.action] || "bg-gray-100 text-slate-600"}`}>{l.action}</span>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-600">{l.target || "-"}</td>
                    <td className="px-4 py-3 text-sm text-gray-400">{l.ipAddress || "-"}</td>
                    <td className="px-4 py-3 text-sm text-slate-500">{new Date(l.createdAt).toLocaleString("zh-CN")}</td>
                    <td className="px-4 py-3">
                      {l.detail ? (
                        <button onClick={() => setDetail(l)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title="查看详情"><Eye className="w-5 h-5" /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="text-lg font-semibold text-slate-900">操作详情 · {detail.module} / {detail.action}</h2>
              <button onClick={() => setDetail(null)} className="p-1 text-gray-400 hover:text-slate-600 rounded transition"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              <p className="text-sm text-slate-600 mb-3">操作对象：{detail.target || "-"}</p>
              <pre className="text-xs bg-slate-50 border border-slate-200 rounded-lg p-3 overflow-auto max-h-80 whitespace-pre-wrap break-all">{detail.detail ? JSON.stringify(detail.detail, null, 2) : "无补充信息"}</pre>
            </div>
          </div>
        </div>
      )}
