...
export default function LoginLogsPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">登录日志</h1>
        <p className="text-sm text-slate-500 mt-1">记录所有登录尝试（成功/失败），用于安全审计与异常登录排查</p>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.list.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-50 transition">
                    <td className="px-4 py-3 text-sm font-medium text-slate-900">{l.username || l.operatorName || "-"}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[l.status] || "bg-gray-100 text-slate-600"}`}>
