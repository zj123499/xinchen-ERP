...
export default function SitesPage() {
  ...
  function SortIcon({ col }: { col: string }) {
    if (sortBy !== col) return <ChevronDown className="w-3 h-3 inline-block opacity-30" />;
    return sortOrder === "asc" ? <ChevronUp className="w-3 h-3 inline-block" /> : <ChevronDown className="w-3 h-3 inline-block" />;
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 mb-4">
        <div className="flex items-center gap-3 p-4 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" placeholder="搜索站点名称或域名..." value={keyword} onChange={e => setKeyword(e.target.value)} onKeyDown={e => e.key === "Enter" && setPage(1)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          ...
          <input type="text" placeholder="云账号筛选..." value={cloudAccountFilter} onChange={e => setCloudAccountFilter(e.target.value)} onKeyDown={e => e.key === "Enter" && setPage(1)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-40" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                <button onClick={() => toggleSort("name")} className="hover:text-gray-900">站点名称 <SortIcon col="name" /></button>
              </th>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                <button onClick={() => toggleSort("domainExpiresAt")} className="hover:text-gray-900">过期时间 <SortIcon col="domainExpiresAt" /></button>
              </th>
            ...
            {loading ? (
              ...
              return (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">{s.cloudAccount || "-"}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{s.resolvedServer ? `${s.resolvedServer.name} (${s.resolvedServer.address})` : "-"}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[s.status] || "bg-gray-100 text-gray-600"}`}>{STATUS_MAP[s.status] || s.status}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => openEditForm(s)} className="p-1.5 text-gray-400 hover:text-blue-600 rounded" title="编辑"><Edit3 className="w-4 h-4" /></button>
                    <button onClick={() => handleDelete(s)} className="p-1.5 text-gray-400 hover:text-red-600 rounded" title="删除"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              );
            })}
        ...
        <div className="flex items-center justify-between p-4 border-t bg-gray-50">
          <div className="flex items-center gap-3 text-sm text-gray-500">
            共 <span className="font-bold text-gray-900">{data?.total || 0}</span> 条记录，第 {page} 页
            <select value={pageSize} onChange={e => { setPageSize(parseInt(e.target.value)); setPage(1); }} className="px-2 py-1 border border-gray-300 rounded text-sm">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} className="p-1.5 border rounded disabled:opacity-50"><ChevronLeft className="w-4 h-4" /></button>
            <span className="px-3 text-sm">{page} / {data?.totalPages || 1}</span>
            <button onClick={() => setPage(p => Math.min(data?.totalPages || 1, p + 1))} disabled={page >= (data?.totalPages || 1)} className="p-1.5 border rounded disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">站点名称 <span className="text-red-500">*</span></label><input type="text" value={formData.name} onChange={e => setFormData(f => ({ ...f, name: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">域名 <span className="text-red-500">*</span></label><input type="text" value={formData.domain} onChange={e => setFormData(f => ({ ...f, domain: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="如:www.example.com" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">使用公司模板</label>
              ...
              <div className="border-t pt-4">
                <h3 className="text-sm font-medium text-gray-700 mb-3">公司信息</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs text-gray-600 mb-1">备案公司</label><input type="text" value={formData.icpCompany} onChange={e => setFormData(f => ({ ...f, icpCompany: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                  <div><label className="block text-xs text-gray-600 mb-1">法人代表</label><input type="text" value={formData.legalRepresentative} onChange={e => setFormData(f => ({ ...f, legalRepresentative: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                </div>
              </div>
              <div className="border-t pt-4">
                <h3 className="text-sm font-medium text-gray-700 mb-3">云账号 & 百度账号</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs text-gray-600 mb-1">云账号</label><input type="text" value={formData.cloudAccount} onChange={e => setFormData(f => ({ ...f, cloudAccount: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                  <div><label className="block text-xs text-gray-600 mb-1">密码</label><input type="password" value={formData.cloudAccountPassword} onChange={e => setFormData(f => ({ ...f, cloudAccountPassword: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                  <div><label className="block text-xs text-gray-600 mb-1">登录验证手机号</label><input type="text" value={formData.cloudLoginPhone} onChange={e => setFormData(f => ({ ...f, cloudLoginPhone: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                  <div><label className="block text-xs text-gray-600 mb-1">百度统计账号</label><input type="text" value={formData.baiduAnalyticsAccount} onChange={e => setFormData(f => ({ ...f, baiduAnalyticsAccount: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                  <div className="col-span-2"><label className="block text-xs text-gray-600 mb-1">百度搜索资源平台账号</label><input type="text" value={formData.baiduSearchResourceAccount} onChange={e => setFormData(f => ({ ...f, baiduSearchResourceAccount: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                </div>
              </div>
              <div className="border-t pt-4">
                <h3 className="text-sm font-medium text-gray-700 mb-3">解析 & 到期</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs text-gray-600 mb-1">解析到服务器</label>
                  ...
                  <div><label className="block text-xs text-gray-600 mb-1">域名过期时间</label><input type="date" value={formData.domainExpiresAt} onChange={e => setFormData(f => ({ ...f, domainExpiresAt: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div>
                </div>
              </div>
