...
const STATUS_COLOR: Record<string, string> = {
  ...
  completed: "bg-gray-100 text-slate-600",
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">租房管理</h1>
          <p className="text-sm text-slate-500 mt-1">管理留学生租房订单，追踪入住和退租状态</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="搜索学生姓名或地址..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div className="relative">
            <button onClick={() => setShowFilter(!showFilter)}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border transition ${statusFilter ? "border-blue-300 bg-indigo-50 text-indigo-700" : "border-slate-300 text-slate-700 hover:bg-slate-50"}`}>
              <Filter className="w-5 h-5" /> 状态 {statusFilter && <span className="w-2 h-2 rounded-full bg-blue-500" />}
            </button>
            {showFilter && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-10 p-3 min-w-[160px]">
                ...
                <div className="space-y-1">
                  <button onClick={() => { setStatusFilter(""); setPage(1); setShowFilter(false); }}
                    className={`w-full text-left px-2 py-1 text-sm rounded ${!statusFilter ? "bg-indigo-50 text-indigo-700" : "text-slate-700 hover:bg-slate-50"}`}>全部</button>
                  {Object.entries(STATUS_MAP).map(([k, v]) => (
                    <button key={k} onClick={() => { setStatusFilter(k); setPage(1); setShowFilter(false); }}
                      className={`w-full text-left px-2 py-1 text-sm rounded ${statusFilter === k ? "bg-indigo-50 text-indigo-700" : "text-slate-700 hover:bg-slate-50"}`}>{v}</button>
                  ))}
                </div>
              </div>
            )}
                  ...
                  <tr key={order.id} className="hover:bg-slate-50 transition">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-orange-100 rounded-full flex items-center justify-center flex-shrink-0"><User className="w-5 h-5 text-orange-600" /></div>
                        <div>
                          <div className="text-sm font-medium text-slate-900">{order.student.name}</div>
                          <div className="text-xs text-slate-400 flex items-center gap-0.5"><Phone className="w-3.5 h-3.5" /> {order.student.phone}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm text-slate-700">{order.city}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-700 max-w-[200px] truncate block" title={order.address || ""}>{order.address || "-"}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm text-slate-700">{new Date(order.moveInDate).toLocaleDateString("zh-CN")}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      {order.moveOutDate ? (
                        <div className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm text-slate-700">{new Date(order.moveOutDate).toLocaleDateString("zh-CN")}</span></div>
                      ) : <span className="text-sm text-gray-400">-</span>}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1"><DollarSign className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm font-medium text-slate-900">{CURRENCY_SYMBOL[order.currency] || order.currency}{order.monthlyRent}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-700">{order.paymentMethod ? getDictLabel(paymentMethods, order.paymentMethod) : "-"}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-700">{order.partner?.name || "-"}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[order.status] || "bg-gray-100 text-slate-600"}`}>{STATUS_MAP[order.status] || order.status}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditForm(order)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title="编辑"><Edit3 className="w-5 h-5" /></button>
                        <button onClick={() => setDeleteConfirm(order)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition" title="删除"><Trash2 className="w-5 h-5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingItem ? "编辑租房" : "新增租房"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formData.address} onChange={(e) => setFormData(d => ({ ...d, address: e.target.value }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="如: 123 Oxford Street, London" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">入住日期 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">月租金 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">合作方</label>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700">确定要删除 {deleteConfirm.student.name} 在 {deleteConfirm.city} 的租房订单吗？</p>
