...
export default function AuditLogsPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">操作审计日志</h1>
        <p className="text-sm text-slate-500 mt-1">记录系统所有关键操作，支持合规追溯与安全审计</p>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.list.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600">{l.tableName || "-"}</td>
                    <td className="px-4 py-3 text-sm text-slate-500">{l.recordId ?? "-"}</td>
                    ...
                    <td className="px-4 py-3">
                      {(l.beforeData || l.afterData) ? (
                        <button onClick={() => setDetail(l)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title="查看变更"><Eye className="w-5 h-5" /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="text-lg font-semibold text-slate-900">变更详情 · {detail.action} {detail.tableName}</h2>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <pre className="text-xs bg-slate-50 border border-slate-200 rounded-lg p-3 overflow-auto max-h-80 whitespace-pre-wrap break-all">{detail.beforeData ? JSON.stringify(detail.beforeData, null, 2) : "无"}</pre>
              </div>
              <div>
