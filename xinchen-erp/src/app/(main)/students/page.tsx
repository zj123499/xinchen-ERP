  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">学生档案</h1>
          <p className="text-sm text-slate-500 mt-1">管理所有学生档案，查看360°全景信息</p>
        </div>
        <button onClick={openNewForm}
          className="flex items-center gap-1.5 px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600">
          <Plus className="w-5 h-5" />新建学生
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setPage(1); fetchStudents(); }}
            className="flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-slate-700 rounded-lg text-sm hover:bg-gray-200">
                ...
                {data.list.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50 cursor-pointer transition"
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600">
                      <div className="flex items-center gap-1"><Phone className="w-3.5 h-3.5 text-gray-400" />{s.phone || "-"}</div>
                      {s.wechat && <div className="text-xs text-slate-400">微信: {s.wechat}</div>}
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-700">
                      {s.targetCountry || "-"} {s.targetDegree ? `/ ${s.targetDegree}` : ""}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_MAP[s.currentStatus || ""]?.color || "bg-gray-100 text-slate-800"}`}>
                        {STATUS_MAP[s.currentStatus || ""]?.label || s.currentStatus || "未知"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-700">{s.assignedTo?.realName || "-"}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span title="线索">{s._count.leads}</span>
                        <span title="合同"><FileText className="w-3.5 h-3.5" />{s._count.contracts}</span>
                        <span title="申请"><ClipboardCheck className="w-3.5 h-3.5" />{s._count.applications}</span>
                      </div>
                    </td>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t bg-slate-50">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
                  className="p-1.5 rounded text-gray-500 hover:bg-gray-200 disabled:opacity-30"><ChevronLeft className="w-5 h-5" /></button>
                <button onClick={() => setPage((p) => p + 1)}
                  className="p-1.5 rounded text-gray-500 hover:bg-gray-200"><ChevronRight className="w-5 h-5" /></button>
              </div>
            </div>
          </>
        )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h3 className="text-lg font-semibold text-slate-900">新建学生</h3>
              <button onClick={() => setShowForm(false)} className="p-1 text-gray-400 hover:text-slate-600 hover:bg-gray-100 rounded"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 text-sm text-slate-600 hover:bg-gray-100 rounded-lg">取消</button>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-sm text-slate-700 mb-1">{deleteTarget.name}</p>
            <p className="text-sm text-slate-500 mb-4">该学生的合同、申请、跟进等关联数据将被一并删除。</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => setDeleteTarget(null)} className="py-2 px-6 border rounded-lg text-sm text-slate-600 hover:bg-slate-50">取消</button>
            </div>
          </div>
        </div>
      )}
...
const ipt = "w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none";
