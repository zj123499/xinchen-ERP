...
export default function AICustomerServicePage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6"><h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Sparkles className="w-6 h-6 text-blue-600" /> AI 客服助手</h1>
        <p className="text-sm text-slate-500 mt-1">自动应答家长常见问题，复杂问题智能转人工，支持多轮对话与保存</p></div>

      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 h-[560px] flex flex-col">
        <div className="p-3 border-b flex items-center gap-2">
          <User className="w-5 h-5 text-gray-400" />
          <select value={studentId} onChange={(e) => setStudentId(Number(e.target.value))} className="flex-1 px-2 py-1 border border-slate-300 rounded-lg text-sm bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {msgs.map((m, i) => m.role === "user" ? (
            ...
            <div key={i} className="flex justify-start">
              <div className={`max-w-[80%] rounded-2xl rounded-bl-sm px-4 py-2.5 text-sm ${m.needHuman ? "bg-amber-50 border border-amber-200 text-amber-800" : "bg-gray-100 text-slate-700"}`}>
                {m.fallback && <div className="text-xs text-slate-400 mb-1">（模板模式）</div>}
                {m.content}
                {m.needHuman && <div className="mt-1 flex items-center gap-1 text-xs text-amber-600"><UserCheck className="w-3.5 h-3.5" /> 建议转人工</div>}
              </div>
            </div>
          ))}
          {loading && <div className="flex justify-start"><div className="bg-gray-100 rounded-2xl px-4 py-2.5 text-sm text-gray-400"><Sparkles className="w-5 h-5 inline animate-pulse" /> 思考中...</div></div>}
        </div>
        <form onSubmit={handleSubmit} className="border-t p-4 space-y-2">
          <input value={context} onChange={(e) => setContext(e.target.value)} placeholder="补充背景（选填，如：学生已签约英国硕士）" className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-xs" />
          <div className="flex gap-2">
            <input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="输入家长问题，回车发送" className="flex-1 px-3 py-2 border border-slate-300 rounded-lg text-sm" />
            <button type="submit" disabled={loading || !question.trim()} className="flex items-center justify-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-60"><Send className="w-5 h-5" />发送</button>
          </div>
        </form>
      </div>
    </div>
