  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-900">风险看板</h1><p className="text-sm text-slate-500 mt-1">主动预警学生交付风险，降低流失与投诉</p></div>
        <button onClick={runScan} disabled={scanning} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:opacity-60">
          <ScanSearch className={`w-5 h-5 ${scanning ? "animate-spin" : ""}`} /> {scanning ? "扫描中..." : "运行风险扫描"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {stats.map((s) => <div key={s.label} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex items-center gap-3">
          <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${s.color}`}><s.icon className="w-5 h-5" /></div>
          <div><div className="text-2xl font-bold text-slate-900">{s.value}</div><div className="text-xs text-gray-500">{s.label}</div></div>
        </div>)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
            <h2 className="font-semibold text-slate-900">风险记录</h2>
            <button onClick={fetchAll} className="p-1.5 text-gray-400 hover:text-slate-600 rounded"><RefreshCw className="w-5 h-5" /></button>
          </div>
            ...
            : records.length === 0 ? <div className="py-16 text-center text-gray-400 text-sm">暂无风险记录，点击右上角运行扫描</div>
              : <table className="w-full"><thead><tr className="bg-slate-50 text-left">
                ...
                {records.map((r) => <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><div className="font-medium text-slate-900 text-sm">{r.rule.name}</div><div className="text-xs text-gray-500 max-w-[220px] truncate">{r.detail || "-"}</div></td>
                  <td className="px-4 py-3 text-slate-600 text-sm">{r.student?.name || "-"}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{STATUS_META[r.status]}</td>
                  ...
                  <td className="px-4 py-3"><div className="flex items-center gap-1">
                    ...
                    <button onClick={() => ignore(r.id)} className="px-2 py-1 text-xs text-slate-600 bg-gray-100 rounded hover:bg-gray-200">忽略</button></>}
                  </div></td>
                </tr>)}
              </tbody></table>}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-200"><h2 className="font-semibold text-slate-900">风险通知</h2></div>
          <div className="max-h-[520px] overflow-y-auto">
            {notifs.length === 0 ? <div className="py-12 text-center text-gray-400 text-sm">暂无通知</div>
              : notifs.map((n) => <div key={n.id} className={`px-4 py-3 border-b border-gray-100 flex items-start gap-2 ${n.read ? "" : "bg-blue-50/40"}`}>
                <Bell className={`w-5 h-5 mt-0.5 ${n.read ? "text-gray-300" : "text-blue-500"}`} />
                <div className="flex-1"><p className="text-sm text-slate-700">{n.content}</p><p className="text-xs text-slate-400 mt-1">{new Date(n.sentAt).toLocaleString("zh-CN")}</p></div>
