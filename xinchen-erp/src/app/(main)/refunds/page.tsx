  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">退费管理</h1>
          <p className="text-sm text-gray-500 mt-1">登记退费申请，审批通过后按比例扣回相关提成</p>
        </div>
        <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition"><Plus className="w-4 h-4" /> 新增退费</button>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.list.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4"><span className="text-sm text-gray-600 line-clamp-1 max-w-[160px]">{item.reason || "—"}</span></td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">退费金额</label>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">退费原因</label>
                <textarea value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" placeholder="说明退费原因" /></div>
