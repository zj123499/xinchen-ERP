...
const STATUS_COLOR: Record<string, string> = {
  ...
  cancelled: "bg-gray-100 text-slate-600",
};
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">境外服务</h1>
          <p className="text-sm text-slate-500 mt-1">管理留学生境外落地服务，包括签证、接机、住宿等</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="搜索学生姓名或手机号..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div className="relative">
            <button onClick={() => setShowFilter(!showFilter)}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border transition ${(typeFilter || statusFilter) ? "border-blue-300 bg-indigo-50 text-indigo-700" : "border-slate-300 text-slate-700 hover:bg-slate-50"}`}>
              <Filter className="w-5 h-5" /> 筛选 {(typeFilter || statusFilter) && <span className="w-2 h-2 rounded-full bg-blue-500" />}
            </button>
            {showFilter && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-10 p-3 min-w-[200px]">
                <div className="text-xs font-medium text-gray-500 mb-2">服务类型</div>
                <div className="space-y-1 mb-3">
                  <button onClick={() => { setTypeFilter(""); setPage(1); }}
                    className={`w-full text-left px-2 py-1 text-sm rounded ${!typeFilter ? "bg-indigo-50 text-indigo-700" : "text-slate-700 hover:bg-slate-50"}`}>全部</button>
                  {serviceTypes.map(t => (
                    <button key={t.dictKey} onClick={() => { setTypeFilter(t.dictKey); setPage(1); }}
                      className={`w-full text-left px-2 py-1 text-sm rounded ${typeFilter === t.dictKey ? "bg-indigo-50 text-indigo-700" : "text-slate-700 hover:bg-slate-50"}`}>{t.dictValue}</button>
                  ))}
                </div>
                  ...
                  <tr key={svc.id} className="hover:bg-slate-50 transition">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-teal-100 rounded-full flex items-center justify-center flex-shrink-0"><User className="w-5 h-5 text-teal-600" /></div>
                        <div>
                          <div className="text-sm font-medium text-slate-900">{svc.student.name}</div>
                          <div className="text-xs text-slate-400 flex items-center gap-0.5"><Phone className="w-3.5 h-3.5" /> {svc.student.phone}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${TYPE_COLORS[serviceTypes.findIndex(t => t.dictKey === svc.serviceType) % TYPE_COLORS.length] || "bg-gray-100 text-slate-600"}`}>
                        {getTypeLabel(svc.serviceType)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm text-slate-700">{svc.startDate ? new Date(svc.startDate).toLocaleDateString("zh-CN") : "-"}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-gray-400" /><span className="text-sm text-slate-700">{svc.endDate ? new Date(svc.endDate).toLocaleDateString("zh-CN") : "-"}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        <FileText className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
                        <span className="text-sm text-slate-700 max-w-[200px] truncate" title={renderDetail(svc.detail)}>{renderDetail(svc.detail)}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[svc.status] || "bg-gray-100 text-slate-600"}`}>
                        {STATUS_MAP[svc.status] || svc.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditForm(svc)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title="编辑"><Edit3 className="w-5 h-5" /></button>
                        <button onClick={() => setDeleteConfirm(svc)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition" title="删除"><Trash2 className="w-5 h-5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingItem ? "编辑服务" : "新增服务"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">服务类型 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">开始服务时间</label>
                  <input type="date" value={formData.startDate} onChange={(e) => setFormData(d => ({ ...d, startDate: e.target.value }))}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">结束服务时间</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">详情 (JSON格式)</label>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700">确定要删除 {deleteConfirm.student.name} 的 {getTypeLabel(deleteConfirm.serviceType)} 服务记录吗？</p>
