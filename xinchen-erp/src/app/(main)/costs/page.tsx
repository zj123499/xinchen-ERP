  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">成本管理</h1>
          <p className="text-sm text-slate-500 mt-1">记录和管理各类运营成本，按会计期归类统计</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.list.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition">
                    <td className="px-4 py-3 text-sm text-slate-700">{c.student?.name || "-"}</td>
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-slate-900">¥{Number(c.amount).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</td>
                    <td className="px-4 py-3 text-sm text-slate-600">{c.currency}</td>
                    <td className="px-4 py-3 text-sm text-slate-600">{c.fiscalYear}年{c.fiscalMonth}月</td>
                    <td className="px-4 py-3 text-sm text-slate-500 max-w-[200px] truncate">{c.description || "-"}</td>
                    <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingCost ? "编辑成本" : "新增成本"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" value={studentSearch} onChange={(e) => { setStudentSearch(e.target.value); setSelectedStudent(null); setFormData((d) => ({ ...d, studentId: "" })); }}
                    placeholder="搜索学生（可选）..." className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">成本类型 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" min="0" required value={formData.amount} onChange={(e) => setFormData((d) => ({ ...d, amount: e.target.value }))}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="0.00" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData((d) => ({ ...d, description: e.target.value }))} rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="成本说明..." />
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700">确定要删除 {deleteConfirm.costType} 类型的成本记录（¥{Number(deleteConfirm.amount).toLocaleString()}）吗？</p>
