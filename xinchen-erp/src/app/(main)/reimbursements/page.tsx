    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">管理员工费用报销申请，支持审批和付款流程</p>
        </div>
                ...
                {data.list.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 transition">
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-slate-900">{r.applicant.realName || r.applicant.username}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="inline-flex text-xs font-medium px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">{r.category}</span>
                    </td>
                    <td className="px-4 py-3 text-sm font-semibold text-slate-900">¥{Number(r.amount).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_MAP[r.status]?.color || "bg-gray-100 text-slate-600"}`}>
                        {STATUS_MAP[r.status]?.label || r.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-500 max-w-[200px] truncate">{r.description || "-"}</td>
                    <td className="px-4 py-3 text-sm text-slate-500">{new Date(r.createdAt).toLocaleDateString("zh-CN")}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {r.status === "DRAFT" && (
                          <button onClick={() => handleStatusChange(r, "SUBMITTED")} className="px-2 py-1 text-xs text-yellow-700 bg-yellow-50 hover:bg-yellow-100 rounded transition" title="提交"><Send className="w-3.5 h-3.5" /></button>
                        )}
                        {r.status === "SUBMITTED" && (
                          <span className="px-2 py-1 text-xs text-blue-700 bg-blue-50 rounded inline-flex items-center gap-1">
                            <Send className="w-3.5 h-3.5" />审批中
                          </span>
                        )}
                        {r.status === "APPROVED" && (
                          <button onClick={() => handleStatusChange(r, "PAID")} className="px-2 py-1 text-xs text-blue-700 bg-blue-50 hover:bg-blue-100 rounded transition" title="付款"><CreditCard className="w-3.5 h-3.5" /></button>
                        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingReimbursement ? "编辑报销" : "新增报销"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">申请人 <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">费用类别 <span className="text-red-500">*</span></label>
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0" required value={formData.amount} onChange={(e) => setFormData((d) => ({ ...d, amount: e.target.value }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="报销金额" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">描述</label>
                <textarea value={formData.description} onChange={(e) => setFormData((d) => ({ ...d, description: e.target.value }))} rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="费用说明..." />
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700">确定要删除 {deleteConfirm.applicant.realName || deleteConfirm.applicant.username} 的 {deleteConfirm.category} 报销记录（¥{Number(deleteConfirm.amount).toLocaleString()}）吗？</p>
