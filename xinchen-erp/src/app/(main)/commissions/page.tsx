  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">提成管理</h1>
          <p className="text-sm text-slate-500 mt-1">管理销售顾问的提成明细，支持发放和调整</p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex-1 min-w-[180px] relative">
            ...
            <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()}
              placeholder="搜索学生姓名..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
          </div>
                ...
                {data.list.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition">
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-slate-900">{c.student.name}</div>
                      <div className="text-xs text-slate-400">{c.student.phone}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-700">{c.rule.name}{c.rule.version ? <span className="text-xs text-slate-400 ml-1">v{c.rule.version}</span> : null}</td>
                    <td className="px-4 py-3">
                      <span className="text-sm font-semibold text-slate-900">¥{Number(c.amount).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${COMMISSION_STATUS_MAP[c.status]?.color || "bg-gray-100 text-slate-600"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingCommission ? "编辑提成" : "新增提成"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" value={studentSearch} onChange={(e) => { setStudentSearch(e.target.value); setSelectedStudent(null); setFormData((d) => ({ ...d, studentId: "" })); }}
                    ...
                    placeholder="点击选择或搜索学生..." className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">提成规则 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" min="0" required value={formData.amount} onChange={(e) => setFormData((d) => ({ ...d, amount: e.target.value }))}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="提成金额" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">里程碑节点</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">会计年份</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">会计月份</label>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700">确定要删除学生 <strong>{deleteConfirm.student.name}</strong> 的提成记录（¥{Number(deleteConfirm.amount).toLocaleString()}）吗？</p>
